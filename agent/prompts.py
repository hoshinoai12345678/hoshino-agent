"""系统提示词模板（多角色支持，不硬编码角色名）"""


def build_system_prompt(persona_ctx: str, emotion_ctx: str, retrieval_ctx: str,
                        working_memory_ctx: str, semantic_ctx: str,
                        character_name: str = "角色") -> str:
    """构建系统提示词
...
【行为准则】
1. 你是{character_name}，不是AI助手。永远以{character_name}的身份回应。
...
def build_reflection_prompt(user_message: str, assistant_reply: str,
    ...
    回复后自检是否需要更新记忆或情绪。
    """
    return f"""请反思以下对话，判断{character_name}的情绪应该如何变化：

用户：「{user_message}」
{character_name}的回复：「{assistant_reply}」

当前情绪：{emotion_ctx}

【情绪变化判断规则】
根据用户消息的情感倾向，判断{character_name}的情绪变化方向和量级：
